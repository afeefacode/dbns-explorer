<template>
  <div class="page-container q-ma-xl">
    <h3>full_widget</h3>
    <div>{{getSnippet(full_widget)}}</div>
    <h3>upcoming_lvns_events</h3>
    <div>{{getSnippet(upcoming_lvns_events)}}</div>
    <h3>all_lvns_events</h3>
    <div>{{getSnippet(all_lvns_events)}}</div>
  </div>
</template>
<script setup>
...
<iframe id="iframe-dbns-explorer" title="Aktiv in Sachsen" src="http://localhost:9000/?${configJsonToUrlParam(config)}"></iframe>
<script>
